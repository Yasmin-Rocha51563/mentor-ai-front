...
export default function QuizAvaliacao({ dadosProva, onFinalizarProva, onVoltar }) {
    ...
    const avaliacaoId = dadosProva?.avaliacao?.id || dadosProva?.id;
    const handleVoltarClick = () => {
        if (onVoltar) onVoltar();
        else if (onFinalizarProva) onFinalizarProva(null);
    };
    ...
    const handleSubmit = async (e) => {
        ...
        setCarregando(true);
        try {
                        const resultado = await enviarRespostasAvaliacao(avaliacaoId, respostasAluno, questoes);
        ...
        return (
            <div style={{ padding: '20px', textAlign: 'center', fontFamily: 'sans-serif' }}>
                <h2 style={{ color: '#0056b3' }}>Avaliação Concluída! </h2>
                <p style={{ fontSize: '24px', fontWeight: 'bold' }}>Sua Nota: {nota}</p>
                <button onClick={handleVoltarClick} style={{ padding: '10px 20px', cursor: 'pointer', marginTop: '20px' }}>
    ...
    return (
        <div style={{ padding: '20px', maxWidth: '700px', margin: '0 auto', fontFamily: 'sans-serif', textAlign: 'left' }}>
            ...
            <h2 style={{ textAlign: 'center', color: '#0056b3' }}>Avaliação Diagnóstica: {dadosProva?.area || dadosProva?.avaliacao?.area}</h2>
